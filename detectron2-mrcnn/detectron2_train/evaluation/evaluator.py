...
def onlykeeppersonclass(outputs, height, width):
    ...
    boxes = np.delete(np.array([t.cpu().numpy() for t in boxes]), indx_to_remove, axis=0)
    ...
    boxes = Boxes(torch.tensor(boxes).to('cuda:0'))
...
def inference_on_dataset(
    model, data_loader, evaluator: Union[DatasetEvaluator, List[DatasetEvaluator], None], custom_filter = False
):
    """
    ...
    with ExitStack() as stack:
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            if custom_filter:
                tmp = [onlykeeppersonclass(outputs[0], inputs[0]['height'], inputs[0]['width'])]
                outputs = tmp
