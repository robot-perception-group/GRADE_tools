...
- Install Successfully with **gcc/g++ 4.8**
  ...
  make -j && sudo make install
  ```
  This will install opencv in the `install` folder specified. Then, in the `CMakeLists.txt` of ORBSLAM2 you need to remove line 31 to 37 and add `find_package(OpenCV 2.4.11 EXACT REQUIRED /path)`
